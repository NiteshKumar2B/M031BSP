...
void UART0_Init(void)
{
    ...
    /*
       ...
       when data is received and available in the receive buffer 
       a RDAI can be triggered
       This interrupt indicates that new data has been received and can be read from the buffer.   
    */
    ...
    /* UART_FIFO acts as a temporary storage for outgoing and incoming data */
    ...
    /*
        RX_TIMEOUT_CNT typically refers to the receive timeout count. 
        When a UART receives data, 
        it waits for a certain amount of time before considering the reception complete 
        and moving on to the next task. 
        This waiting period is known as the receive timeout.
    */
...
int32_t main(void)
{
    ...
    while(1)
    {
        /* Wait to receive UART data *//* Wait until UART gets into Working state  */
	...
	// Wait RX Data receving is completed
	...
	printf("\nUART0 Rx Received Data: %s\n",g_au8UART_RX_Buffer);
        ...
        /* Reset SRAM Buffer or Reset all Parameters */
