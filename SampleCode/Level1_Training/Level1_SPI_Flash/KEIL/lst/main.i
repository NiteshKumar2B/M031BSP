...
int main(void)
{
    ...
    GPIO_SetMode(((GPIO_T *) (((( uint32_t)0x40000000) + 0x4000) )), ((0x00000010UL) ///< Bit 4 mask of an 32 bit integer | (0x00000020UL) ///< Bit 5 mask of an 32 bit integer), 0x1UL);
    ...
    for(u32ByteCount=0; u32ByteCount<256; u32ByteCount++) //data count store kr rhe hai
    {
        SrcArray[u32ByteCount] = u32ByteCount; //SrcArray me all data daal dia
    }
    ...
    for(u32PageNumber=0; u32PageNumber<1; u32PageNumber++)
    {
        ...
        SpiFlash_WaitReady();
        u32FlashAddress += 0x100; //256 bhar diya next 256 k liya aage ja raha hai 0x100=256
    }
    ...
    for(u32PageNumber=0; u32PageNumber<1; u32PageNumber++)
    {
        ...
        for(u32ByteCount=0; u32ByteCount<256; u32ByteCount++)
        {
            if(DestArray[u32ByteCount] != SrcArray[u32ByteCount]) //check dest array == source array haiya nhi
                nError ++; // if hai to error ++ kr do
        }
    }
...
void SpiFlash_NormalPageProgram(uint32_t StartAddress, uint8_t *u8DataBuffer)
{
    ...
    while(1)
    {
        ...
        {
            ((((SPI_T *) ((( uint32_t)0x40000000) + 0x61000)))->TX = (u8DataBuffer[i++]));
            if(i >= 255) break;
        }
    }
...
void SpiFlash_NormalRead(uint32_t StartAddress, uint8_t *u8DataBuffer)
{
    ...
    for(i=0; i<256; i++)
    {
        ((((SPI_T *) ((( uint32_t)0x40000000) + 0x61000)))->TX = (0x00));
        while(( ((((SPI_T *) ((( uint32_t)0x40000000) + 0x61000)))->STATUS & (0x1ul << (0)))>>(0) ));
        u8DataBuffer[i] = ((((SPI_T *) ((( uint32_t)0x40000000) + 0x61000)))->RX);
    }
